...
@Slf4j
@SpringBootTest
public class PTAggTicketDBResetInitTests {
    ...
    public void testReset() {
        ...
        CompletableFuture<Void> getResultFuture = CompletableFuture.runAsync(() -> {
        ...
        CompletableFuture.allOf(
                Stream.concat(Stream.of(resetFutures), Stream.of(getResultFuture))
                        .toArray(CompletableFuture[]::new)
        ).join();
    }
    ...
    public void cacheInit() {
        ...
        HttpResponse execute = HttpUtil.createPost("http://127.0.0.1:9000/api/user-service/v1/login")
                ...
                .execute();
        JSONObject userLoginResponseJSONObject = JSON.parseObject(execute.body());
        ...
        for (int i = 5; i < 25; i++) {
            HttpUtil.createPost("http://127.0.0.1:9000/api/ticket-service/ticket/purchase/v2")
                    .body(String.format("""
                            ...
                            """, i))
                    .header("Authorization", userLoginResponseJSONObject.getString("accessToken"))
                    .execute();
        }
